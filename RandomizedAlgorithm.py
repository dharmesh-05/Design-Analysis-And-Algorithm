...
# Fixed string formatting (using double quotes outside, single quotes inside)
print(f"{'Input Type':<16} {'DQS Comps':>12} {'DQS Time(ms)':>14} {'RQS Comps':>12} {'RQS Time(ms)':>14}")

# --- Optional: Added the execution loop so your script actually runs the tests ---
for name, arr in test_cases.items():
    dqs_comps, dqs_time = run_test(name, deterministic_quicksort, arr)
    rqs_comps, rqs_time = run_test(name, randomized_quicksort, arr)
    print(f"{name:<16} {dqs_comps:>12} {dqs_time:>14.2f} {rqs_comps:>12} {rqs_time:>14.2f}")